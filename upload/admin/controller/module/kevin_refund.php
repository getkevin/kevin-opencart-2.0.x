...
class ControllerModuleKevinRefund extends Controller
{
    private $error = [];

    private $lib_version = '0.3';
    private $plugin_version = '1.0.0.1';
    ...
    public function install()
    {
        ...
        if ($this->validate()) {
            $this->model_setting_setting->editSetting('kevin_refund', ['kevin_refund_status' => 1]);
    ...
    public function index()
    {
        ...
        $this->load->model('module/kevin_refund');
    ...
    {
        require_once DIR_CATALOG.'/model/payment/kevin/vendor/autoload.php';
        $clientId = $this->config->get('kevin_client_id');
        $clientSecret = $this->config->get('kevin_client_secret');
        $endpointSecret = $this->config->get('kevin_client_endpointSecret');
...
*/
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].$url, 'SSL'),
        ];

        //	$data['invoice'] = $this->url->link('module/kevin_refund/invoice', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $order_total = $this->model_module_kevin_refund->getTotalOrders($filter_data);
        ...
        foreach ($results as $result) {
            $product_results = $this->model_module_kevin_refund->getOrderProducts($result['order_id']);
            ...
            $query_refunded = $this->model_module_kevin_refund->getRefundOrderAmount($result['order_id']);
            ...
            //	'amount_available'      => $this->currency->format((float)$result['amount_available'], $result['currency_code'], $currency_value),
                ...
                'view' => $this->url->link('module/kevin_refund/info', 'token='.$this->session->data['token'].'&order_id='.$result['order_id'].$url, 'SSL'),
                'refund_kevin' => $this->url->link('module/kevin_refund/refundKevin', 'token='.$this->session->data['token'].'&order_id='.$result['order_id'].$url, 'SSL'),
            ];
        }
        ...
        $texts = [
            ...
            'text_loading',
            'text_restock',
            'text_refund_order_id',
            'text_restock',
            'text_product_restock',
            'text_refund_history',
            'text_refund_available',
            'text_refunded',
            'text_Qty',
            'text_refund_amount',
            'text_reason',
            ...
            'column_total',
            'column_amount',
            'column_amount_available',
            'column_refund_action',
            'column_reason',
            'column_refund_status',
            'column_date_added',
...
*/
        ...
        $data['sort_order'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=o.order_id'.$url, 'SSL');
        $data['sort_customer'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=customer'.$url, 'SSL');
        $data['sort_status'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=order_status'.$url, 'SSL');
        $data['sort_total'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=o.total'.$url, 'SSL');
        $data['sort_total_amount'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=total_amount'.$url, 'SSL');
        $data['sort_amount_available'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=amount_available'.$url, 'SSL');
        $data['sort_refund_action'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=return_action'.$url, 'SSL');
        $data['sort_date_added'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=o.date_added'.$url, 'SSL');
        $data['sort_date_modified'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].'&sort=o.date_modified'.$url, 'SSL');
...
*/
        ...
        $pagination->url = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].$url.'&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('module/kevin_refund_list.tpl', $data));
    }
...
*/
        ...
        // Kevin refund
        if ($order_info) {
            $query_refund = $this->model_module_kevin_refund->getRefundOrder($order_id);

            $data['order_id'] = $order_id;
            $data['refund_kevin'] = $this->url->link('module/kevin_refund/refundKevin', 'token='.$this->session->data['token'].'&refund_info=1&order_id='.$order_id.$url, 'SSL');
            $data['total'] = $query_refund['total'];
            $data['amount_0'] = $this->currency->format('0', $order_info['currency_code'], $order_info['currency_value']);

            $query_refunded = $this->model_module_kevin_refund->getRefundOrderAmount($order_id);
            ...
            $product_results = $this->model_module_kevin_refund->getOrderProducts($order_id);
            ...
            $refund_results = $this->model_module_kevin_refund->getRefundedOrder($order_id);
            ...
            $results = $this->model_module_kevin_refund->getRestockedProducts($order_id);
            //$this->model_module_kevin_refund->getRestockProducts($order_id);	//test
...
*/
            ...
            //	$data['invoice'] = $this->url->link('module/kevin_refund/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');

            $data['cancel'] = $this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].$url, 'SSL');
            ...
            $this->response->setOutput($this->load->view('module/kevin_refund_info.tpl', $data));
        } else {
    ...
    public function checkQuantity()
    {
        ...
        if (isset($this->request->get['order_id']) && isset($this->request->get['products'])) {
            $get_products = json_decode(html_entity_decode($this->request->get['products']), 'SSL');
            $restock = [];
            $product = [];
            $restock_quantity = [];
            ...
            $product_query = $this->model_module_kevin_refund->getRestockedProducts($this->request->get['order_id']);

            $query_currency = $this->model_module_kevin_refund->getKevinOrderCurrency($this->request->get['order_id']);
...
*/
        ...
        if (!empty($this->request->get['order_id']) && !empty($this->request->post['payment_id'])) {
            ...
            if (empty($this->request->post['kevin_refund_amount'])) {
                $this->session->data['error_refund'] = 'An error occurred! Refund amount was not entered. Please try again by entering the Refund Amount.';
                $this->KevinRefundLog($this->session->data['error_refund']);
                if ($refund_info) {
                    $this->response->redirect($this->url->link('module/kevin_refund/info&order_id='.$order_id, 'token='.$this->session->data['token'], 'SSL'));
                } else {
                    $this->response->redirect($this->url->link('module/kevin_refund', 'token='.$this->session->data['token'].$url, 'SSL'));
                }
            }

            $paymentId = $this->request->post['payment_id'];

            $kevin_order_info = $this->model_module_kevin_refund->getKevinOrder($paymentId);
            ...
            if (!empty($_SERVER['HTTPS']) || 1 == $_SERVER['HTTPS']) {
                $webhook_url = HTTPS_CATALOG.'index.php?route=payment/kevin/webhookRefund';
            } else {
                $webhook_url = HTTP_CATALOG.'index.php?route=payment/kevin/webhookRefund';
            }
            ...
            $this->model_module_kevin_refund->addKevinRefund($refund);

            $this->session->data['success_refund'] = 'Amount prepared for refund: '.$refunded_amount.'. Refund status: "'.$refund['statusGroup'].'". Order No. '.$order_id.'.';
            ...
            $this->session->data['error_refund'] = 'An error occurred! Please check the form and fill in all required fields and try again.';
            $this->KevinRefundLog($this->session->data['error_refund']);
    ...
    {
        if (!$this->user->hasPermission('modify', 'module/kevin_refund')) {
