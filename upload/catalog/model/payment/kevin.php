...
class ModelPaymentKevin extends Model
{
    ...
    public function getMethod($address, $total)
    {
        ...
        if ($this->config->get('kevin_total') > 0 && $this->config->get('kevin_total') > $total) {
            $status = false;
        ...
        if ('right' == $this->config->get('kevin_position')) {
    ...
    public function updateWebhookKevinRefund($data)
    {
        ...
        if ('completed' != $data['statusGroup']) {
            $this->db->query('UPDATE '.DB_PREFIX."kevin_order SET refund_action_id = '".(int) $this->config->get('kevin_created_action_id')."' WHERE payment_id = '".$this->db->escape($data['payment_id'])."'");
        } elseif ($total_amount == $total && 'completed' == $data['statusGroup']) {
            $this->db->query('UPDATE '.DB_PREFIX."kevin_order SET refund_action_id = '".(int) $this->config->get('kevin_refunded_action_id')."' WHERE payment_id = '".$this->db->escape($data['payment_id'])."'");
            $order_status = $this->config->get('kevin_refunded_status_id');
        } else {
            $this->db->query('UPDATE '.DB_PREFIX."kevin_order SET refund_action_id = '".(int) $this->config->get('kevin_partial_refund_action_id')."' WHERE payment_id = '".$this->db->escape($data['payment_id'])."'");
            $order_status = $this->config->get('kevin_partial_refund_status_id');
        }
