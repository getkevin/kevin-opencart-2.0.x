...
class ControllerPaymentKevin extends Controller
{
    ...
    public function index()
    {
        ...
        require_once DIR_APPLICATION.'/model/payment/kevin/vendor/autoload.php';
        ...
        } elseif (file_exists(DIR_APPLICATION.'controller/payment/kevin_image/credit_card_icon.png')) {
        ...
        $data['action'] = $this->url->link('payment/kevin/redirect', 'SSL');
        ...
        } elseif (file_exists(DIR_TEMPLATE.'default/template/'.$this->type.'/'.$this->name.'.tpl')) {
    ...
    public function redirect()
    {
        ...
        $this->model_payment_kevin->addKevinOrder($add_order);
    ...
    public function webhook()
    {
        ...
        if ($isValid) {
            ...
            $order_query = $this->model_payment_kevin->getKevinOrders($payment_id);
            $update_order_signature['order_status_id'] = $this->config->get('kevin_pending_status_id');
            $update_order_signature['payment_id'] = $payment_id;

            if ($order_query['order_status_id'] != $update_order_signature['order_status_id']) {
                $this->model_payment_kevin->updateSignatureKevinOrder($update_order_signature);
                ...
                $this->model_checkout_order->addOrderHistory($order_query['order_id'], $this->config->get('kevin_pending_status_id'), $comment_sign, true);
            }
        ...
        if ($old_status_id != $new_status_id && $order_info['order_id'] == $order_id) {
            $this->KevinLog($log_data);
            $this->model_payment_kevin->updateWebhookKevinOrder($update_order);
            $payment_status = true;
        } else {
    ...
    public function webhookRefund()
    {
        ...
        $refund_query = $this->model_payment_kevin->getKevinRefunds($payment_id, $refund_id);
        ...
        if ($old_status_id != $get_refund_status['statusGroup'] && $order_info['order_id'] == $order_id && $isValid) {
            ...
            $query_refunded_order_status = $this->model_payment_kevin->updateWebhookKevinRefund($update_order);
            $payment_status = true;
            echo 'Signatures match.';
